...
!    if( IO_L ) write(IO_FID_LOG,*), 'judge:', angle_aob-(angle_aop+angle_pob), angle_cod-(angle_cop+angle_pod)
    ...
    ! --- judge intersection
    if (       abs(angle_aob-(angle_aop+angle_pob)) < CONST_EPS &
         .AND. abs(angle_cod-(angle_cop+angle_pod)) < CONST_EPS &
         .AND. abs(angle_aop) > CONST_EPS                       &
         .AND. abs(angle_pob) > CONST_EPS                       &
         .AND. abs(angle_cop) > CONST_EPS                       &
         .AND. abs(angle_pod) > CONST_EPS                       ) then
       ifcross = .true.
    else
       ifcross = .false.
       p(:) = 0.0_RP
...
!ESC!       EPS => CONST_EPS
    ...
    do i = j+1, nvert
       ...
       if ( ip < -CONST_EPS ) then ! right hand : exchange
...
!    if( IO_L ) write(IO_FID_LOG,*) ip, angle1, angle2, abs(angle1)-abs(angle2)
    ...
    if (       abs(ip)                 < CONST_EPS  &      ! on the same line
...
!ESC!       EPS => CONST_EPS
    ...
    elseif( polygon_type == 'ON_SPHERE' ) then ! On a unit sphere, area = sum of angles - pi.
       ...
       if ( abab < CONST_EPS .OR. acac < CONST_EPS ) then
       ...
       if ( bcbc < CONST_EPS .OR. baba < CONST_EPS ) then
       ...
       if ( caca < CONST_EPS .OR. cbcb < CONST_EPS ) then
